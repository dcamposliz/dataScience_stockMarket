...
nasdaq_TR1 <- dataMaster_TR1[, 'nasdaq']
nasdaq_TR1
# turning this vector into ts object
act_nasdaq_2014_ts <- ts(dataMaster_TS1$nasdaq, start=c(2014, 1), freq=12)
act_nasdaq_2014_ts
plot(act_nasdaq_2014_ts)

#  attributes(for_nasdaq_mod) 
for_nasdaq_all <- forecast(auto.arima(nasdaq_TR), 24)
for_nasdaq_all
for_nasdaq_df <- data.frame(for_nasdaq_all)
for_nasdaq_vals <- for_nasdaq_df$Point.Forecast
for_nasdaq_2014_ts <- ts(for_nasdaq_vals, start = c(2014, 1), freq = 12) 
for_nasdaq_2014_ts
plot(for_nasdaq_2014_ts)
###Forecast data for 2015 using ARIMA model
#only getting the information for the year 2015
...
lo <- data.frame(for_nasdaq_all$lower)
lo_df <- lo$X95.
for_nasdaq_lo95CI_14ts <- ts(lo_df, start=c(2014, 1), freq = 12)
for_nasdaq_lo95CI_14ts
plot(for_nasdaq_lo95CI_14ts)
#including upper 95 % CI Lines
up <- data.frame(for_nasdaq_all$upper)
up_df <- up$X95. 
for_nasdaq_up95CI_14ts <- ts(up_df, start = c(2014, 1), freq = 12)
for_nasdaq_up95CI_14ts
plot(for_nasdaq_up95CI_14ts)
#real time data for 2014-15
plot.ts(act_nasdaq_2014_ts)
together_df <- data.frame(for_nasdaq_2014_ts, act_nasdaq_2014_ts, for_nasdaq_lo95CI_14ts, for_nasdaq_up95CI_14ts, date = seq.Date(as.Date("2014-01-01"), by="1 month", length.out=24))
together_df

par(mar = rep(2, 4))
ggplot(together_df, aes(date)) + 
  geom_ribbon(aes(x=date, ymax= up_df, ymin= lo_df), fill="gray", alpha=.5) +
  geom_line(aes(y = for_nasdaq_2014_ts, colour = "Forecasted Values for 2015")) + 
  geom_line(aes(y = act_nasdaq_2014_ts, colour = "Actual Values for 2015")) +
  geom_line(aes(y = lo_df, colour = "Lower 95% Bound"), linetype = 'dotted') +
  geom_line(aes(y = up_df, colour = "Upper 95% Bound"), linetype = 'dotted') +
  scale_color_manual(name="Groups" ,values=c("green", "blue", "red", "red"))
